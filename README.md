...
Cats-Actors leverages Cats Effect for managing effects and supports a generic `F[_]` type, allowing interoperability
with other effect types.
...
import com.suprnation.actor.{Actor, ActorSystem, Props, ActorRef, InternalActorRef}
import com.suprnation.actor.Actor.Receive

class HelloWorldActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case "hello" => IO.println("Hello, World!")
    case _ => IO.unit
  }
}
```
...
object HelloWorldApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        actorRef <- system.actorOf(Props[IO](new HelloWorldActor), "helloWorldActor")
        _ <- actorRef ! "hello"
...
- `HelloWorldActor extends Actor[IO]`, meaning it processes messages within the `IO` effect.
- The receive method is overridden to handle incoming messages. Here, it simply prints "Hello, World!" when it receives
  the message "hello".
...
- `system.actorOf(Props[IO](new HelloWorldActor), "helloWorldActor")` creates an instance of HelloWorldActor within the
  actor system.
- `actorRef ! "hello"` sends the "hello" message to the actor.
- `system.waitForTermination` waits for the actor system to terminate. This ensures that all actors have completed their
  work before the application exits.
...
Below is a comprehensive example of a Hello World application using Cats-Actors, combining the concepts from the
provided files and using the `?` syntax for querying an actor.

First, let's define an actor that will handle the "Hello World" message and respond to queries.
...
class HelloWorldActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case "hello" => IO.println("Hello, World!") >> IO.pure("Hello, World!")
  }
}
```
...
object HelloWorldApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        actorRef <- system.actorOf(Props[IO](new HelloWorldActor), "helloWorldActor")
        response <- actorRef ?[String] "hello"
...
- `HelloWorldActor extends Actor[IO]`, meaning it processes messages of type String within the IO effect.
- The receive method is overridden to handle incoming messages. Here, it prints "Hello, World!" and returns the same
  string when it receives the message "hello".
...
- `system.actorOf(Props[IO](new HelloWorldActor), "helloWorldActor")` creates an instance of HelloWorldActor within the
  actor system.
- `actorRef ? [String] "hello"` sends the "hello" message to the actor and waits for a response. _Note that the ask syntax
  is also typed._
- `IO.println(s"Received response: $response")` prints the response received from the actor.
- The actor system is automatically cleaned up after the use block completes.

> **Note:** that if you do not care about the reply but only care that the actor has processed the message you can use the `?!`
> sytnax. We can rewrite the above example as
...
the actor when using the ? syntax. The ? syntax sends a message to the actor and waits for a response, ensuring that the
...
Below is a comprehensive example of an actor that receives multiple messages forever. We will use Cats-Effect to
leverage fibers for concurrent message sending in the background.

First, define an actor that will handle multiple messages.

```scala
import cats.effect.IO
import com.suprnation.actor.{Actor, ActorSystem, Props, ActorRef}
import com.suprnation.actor.Actor.Receive

class EchoActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case message => IO.println(s"Received message: $message")
  }
}
```
...
object EchoApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        actorRef <- system.actorOf(Props[IO](new EchoActor), "echoActor")
...
- EchoActor extends `Actor[IO]`, meaning it processes messages within the IO effect.
...
- `actorSystemResource.use { system => ... }` uses the .use method to manage the lifecycle of the actor system.
- `system.actorOf(Props[IO](new EchoActor), "echoActor")` creates an instance of EchoActor within the actor system.
- `sendMessagesForever(actorRef).start` starts sending messages to the actor in a background fiber.
- The actor system waits until an internal error happens, in such an eventuality the actor system is cleaned up after
  the use block completes.

3. Send Messages Forever:

- `sendMessagesForever` is a function that sends a list of messages to the actor repeatedly.
- `messages.traverse_(msg => actorRef ! msg >> IO.sleep(1.second))` sends each message to the actor with a 1-second delay
  between messages.
- `sendMessage.foreverM` repeats the message sending process indefinitely.

#### Leveraging Cats-Effect

By using Cats-Effect, we can leverage its powerful concurrency primitives to create complex use cases like this one. The
start method allows us to run the message-sending process in a background fiber, ensuring that the actor continues to
receive messages indefinitely without blocking the main application flow. The foreverM method is used to repeat the
message-sending process forever, demonstrating how Cats-Effect can be used to build highly concurrent and resilient
applications.

### Using cats-effects to send parallel messages to an actor

Below is a comprehensive example of an actor that receives multiple messages in parallel. We will use Cats-Effect to
leverage fibers for concurrent message sending.
...
- `_ <- system.waitForTermination` keeps the actor system alive and waiting for messages unless the actor system itself
  dies internally.
- The actor system is automatically cleaned up after the use block completes.
...
- `messages.parTraverse_(msg => actorRef ! msg)` sends each message to the actor concurrently using `parTraverse_` from
  Cats, which runs the effects in parallel.

#### Leveraging Cats-Effect for Concurrency

By using Cats-Effect, we can leverage its powerful concurrency primitives to create complex use cases like this one. The
parTraverse_ method allows us to run the message-sending process in parallel, ensuring that the actor receives messages
concurrently.

## Creating Actors

Below are three examples demonstrating how to create multiple actors in different ways using the the `cats-actors`
library.
...
object SystemActorApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        actorRef1 <- system.actorOf(Props[IO](new EchoActor), "echoActor1")
        actorRef2 <- system.actorOf(Props[IO](new EchoActor), "echoActor2")
        actorRef3 <- system.actorOf(Props[IO](new EchoActor), "echoActor3")
...
class ParentActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case "createChildren" =>
      for {
        child1 <- context.system.actorOf(Props[IO](new EchoActor), "childActor1")
        child2 <- context.system.actorOf(Props[IO](new EchoActor), "childActor2")
...
object ParentActorApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        parentActor <- system.actorOf(Props[IO](new ParentActor), "parentActor")
...
class GrandParentActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case "createParent" =>
      for {
        parent <- context.system.actorOf(Props[IO](new ParentActor), "parentActor")
...
class ParentActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case "createChild" =>
      for {
        child <- context.system.actorOf(Props[IO](new EchoActor), "childActor")
...
object GrandParentActorApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        grandParentActor <- system.actorOf(Props[IO](new GrandParentActor), "grandParentActor")
...
- _Note: The `GrandParentActor` is the parent of the `ParentActor`, and the `ParentActor` is the parent of the `EchoActor`. Each
  parent actor is responsible for the supervision of its child actors._
...
- **Kill**
  ...
  - **Behavior**: When an actor receives a Kill message, it throws an exception (typically an ActorKilledException),
    causing the actor to immediately terminate and restart if supervised.
...
- **context.self.stop**
  ...
  - **Behavior**: Immediately stops the actor. Any messages in the mailbox are discarded, and the actor transitions to
    the
    stopped state.
  - **Use Case**: Suitable for scenarios where the actor decides to terminate itself after completing its work or
    encountering a condition that requires it to stop.

Let's illustrate these with examples

### Example 1: Killing an Actor Using context.self.stop

In this example, we define an actor that stops itself when it receives a specific message.
...
class SelfStoppingActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case "stop" =>
...
object SelfStoppingActorApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    val actorSystemResource: Resource[IO, ActorSystem[IO]] = ActorSystem[IO]("SelfStoppingActorSystem")

    actorSystemResource.use { system =>
      for {
        actorRef <- system.actorOf(Props[IO](new SelfStoppingActor), "selfStoppingActor")
        _ <- actorRef ! "Hello, Actor!"
        _ <- actorRef ! "stop"
...
```scala
import cats.effect.{IO, IOApp, ExitCode, Resource}
import com.suprnation.actor.{Actor, ActorSystem, Props, ActorRef, PoisonPill}
import com.suprnation.actor.Actor.Receive

class PoisonPillActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case PoisonPill =>
      IO.println("Received PoisonPill, stopping actor...") >> context.self.stop
    case msg: String =>
      IO.println(s"Received message: $msg")
  }
}
...
object PoisonPillActorApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    val actorSystemResource: Resource[IO, ActorSystem[IO]] = ActorSystem[IO]("PoisonPillActorSystem")

    actorSystemResource.use { system =>
      for {
        actorRef <- system.actorOf(Props[IO](new PoisonPillActor), "poisonPillActor")
        _ <- actorRef ! "Hello, Actor!"
        _ <- actorRef ! PoisonPill
        _ <- IO.sleep(1.second) // Give some time for the actor to stop
      } yield ExitCode.Success
    }
  }
...
```scala
import cats.effect.{IO, IOApp, ExitCode, Resource}
import com.suprnation.actor.{Actor, ActorSystem, Props, ActorRef, Kill}
import com.suprnation.actor.Actor.Receive

class KillableActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case Kill =>
      IO.println("Received Kill, stopping actor...") >> context.self.stop
    case msg: String =>
      IO.println(s"Received message: $msg")
  }
}
...
object KillableActorApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        actorRef <- system.actorOf(Props[IO](new KillableActor), "killableActor")
        _ <- actorRef ! "Hello, Actor!"
        _ <- actorRef ! Kill
        _ <- IO.sleep(1.second) // Give some time for the actor to stop
      } yield ExitCode.Success
...
- `SelfStoppingActor`:
  - Stops itself when it receives the "stop" message.
...
- `PoisonPillActor`:
  - Stops itself when it receives a PoisonPill message.
  - Prints a message when it receives any other message.
- `PoisonPillActorApp`:
...
- `KillableActorApp` creates an actor system, sends a message to the actor, and then sends the `Kill` message to stop the
  actor.

These examples demonstrate how to kill actors using both `context.self.stop`, `PoisonPill` and `Kill` in the `cats-actors`
library. Let's understand the difference here
...
````scala
class RespondingActor extends Actor[IO] {
  override def receive: Receive[IO] = {
    case msg =>
      context.sender ! s"Received your message: $msg"
  }
}
````
...
```scala
class ChildActor extends Actor[IO] {
  def receive: Receive[IO] = {
    case msg =>
...
case class ParentActor(childRef: ActorRef[IO]) extends Actor[IO] {
  override def receive: Receive[IO] = {
    case Message(msg) =>
...
object ActorContextExampleApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        childActor <- system.actorOf(Props[IO](ChildActor()), "child-actor")
        parentActor <- system.actorOf(Props[IO](ParentActor(childActor)), "parent-actor")
...
- `ParentActor`: Receives messages and prints the message along with the sender (`context.sender`). It forwards messages to
  the `ChildActor` and handles `GetParent` and `GetSender` messages to print the parent actor and the sender, respectively.
...
- It sends `GetParent` and `GetSender` messages to the `ChildActor` to demonstrate the usage of `context.parent` and
  `context.sender`.

> **Note:** When the main app sends a message, sender is None. This is normal as there is no ActorRef from the main app.

## Forwarding of Messages in Actor Systems

Forwarding messages in an actor system means sending a message from one actor to another on behalf of the original
sender. This is useful when an actor needs to delegate tasks to other actors without altering the sender information,
maintaining the original sender's identity for responses or acknowledgments.
...
case class ForwardActor(forwardTo: ActorRef[IO], ref: Ref[IO, Option[ActorRef[IO]]]) extends Actor[IO] {
  override def receive: Receive[IO] = {
...
case class BaseActor(ref: Ref[IO, Option[ActorRef[IO]]]) extends Actor[IO] {
  override def receive: Receive[IO] = {
    case ForwardMessage(msg) =>
      ref.set(sender) >> IO.println(s"BaseActor received message: $msg")
  }
}
```
...
object ForwardExampleApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        ref <- Ref.of[IO, Option[ActorRef[IO]]](None)
        baseActor <- system.actorOf(Props[IO](BaseActor(ref)), "base-actor")
        forwardActor <- system.actorOf(Props[IO](ForwardActor(baseActor, ref)), "forward-actor")
...
```scala
object ForwardSyntaxExampleApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    val actorSystemResource = ActorSystem[IO]("ForwardSyntaxExampleSystem")
    ...
    actorSystemResource.use { system =>
      for {
        ...
        // Send a message to the forward actor, which will forward it to the base actor using >>!
        _ <- forwardActor >>! ForwardMessage("Hello using >>!")
        _ <- system.waitForIdle()
      } yield ExitCode.Success
    }
  }
...
> **Note:** When an actor forwards a message, `context.sender` remains the original sender, allowing the receiving actor
> to respond directly to the original sender if needed.

## Supervision

Supervision in an actor system refers to the mechanism by which parent actors manage the lifecycle and failures of their
child actors. It ensures fault tolerance and robust error handling in the system.

### Key Concepts

- Parent-Child Hierarchy: Actors are organized in a hierarchical structure where parent actors supervise their child
  actors.
...
- Supervision Strategies
  - `Restart Strategy`: Commonly used to handle transient errors. The actor's state is re-initialized, and it can resume
    processing new messages.
  - `Stop Strategy`: Used when an actor encounters an unrecoverable error or when it no longer needs to process
    messages.
  - `Resume Strategy`: Allows the actor to continue processing messages without restarting, used for recoverable errors
    that don't require state re-initialization.
  - `Escalate Strategy`: Passes the failure to the parent actor to decide the next course of action.

Below is an example demonstrating supervision in an actor system using the `cats-actors` library. This example will cover
the key concepts and supervision strategies you mentioned.
...
import com.suprnation.actor.props.Props
import com.suprnation.actor.{Actor, ActorSystem, OneForOneStrategy, SupervisionStrategy}

import scala.concurrent.duration.DurationInt
import scala.language.postfixOps

case class ParentActor() extends Actor[IO] {
  ...
  override def preStart: IO[Unit] =
    context.actorOf(Props(ChildActor()), "child-actor").void

  override def receive: Receive[IO] = {
    case msg =>
      for {
        children <- context.children
        _ <- children.headOption.fold(IO.unit)(child => child ! msg)
      } yield ()
...
object SupervisionExampleApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        parentActor <- system.actorOf(Props[IO](ParentActor()), "parent-actor")
        _ <- parentActor ! "Hello, Actor!"
        _ <- parentActor ! "fail" // This will cause the child actor to restart
        _ <- parentActor ! "Hello again!"
        _ <- parentActor ! "illegal" // This will cause the child actor to stop
        _ <- parentActor ! "Are you there?"
...
- `ParentActor`:
  ...
  - Creates a child actor in the `preStart` method.
...
- `SupervisionExampleApp`:
  ...
  - Sends another normal message to check if the child actor is still there. Actor will not be there and message is
    re-routed to the `dead-letter` queue.

### Supervision Strategies Demonstrated

- **Restart**: When the `ChildActor` receives the "fail" message, it raises a `RuntimeException`, causing the `ParentActor` to
  restart it.
- **Stop**: When the `ChildActor` receives the "illegal" message, it raises an `IllegalArgumentException`, causing the
  `ParentActor` to stop it.
- **Escalate**: Any other exceptions would be escalated to the parent actor, but this is not demonstrated in this
  example.

This example demonstrates how to implement and use supervision strategies in an actor system using the `cats-actors`
library.
...
- `preSuspend`
  ...
  > **Note:** Only the actor which caused the errorwill receive the message. This is to ensure that no actor receives messages which are not intended for them
  > and thus result in information leak.
  >

### Example: Actor hooks

Let's update the example to make use of the lifecycle hooks such as `preStart`, `postStop`, `preRestart`, `postRestart`
and `preSuspend`. These hooks allow you to define custom behavior at different stages of an actor's lifecycle.
...
case class ChildActor() extends Actor[IO] {
  ...
  override def preRestart(reason: Throwable, message: Option[Any]): IO[Unit] =
    for {
      _ <- IO.println(s"ChildActor is restarting due to: ${reason.getMessage}")
      _ <- super.preRestart(reason, message)
    } yield ()

  override def postRestart(reason: Throwable): IO[Unit] =
    for {
      _ <- IO.println(s"ChildActor has restarted due to: ${reason.getMessage}")
      _ <- super.postRestart(reason)
    } yield ()

  override def preSuspend(reason: Option[Throwable], message: Option[Any]): IO[Unit] =
    for {
      _ <- IO.println(s"ChildActor is being suspended due to: ${reason.map(_.getMessage).getOrElse("unknown reason")}")
...
case class ParentActor() extends Actor[IO] {
  ...
  override def preStart: IO[Unit] =
    for {
      ...
      _ <- context.actorOf(Props(ChildActor()), "child-actor").void
    } yield ()
  ...
  override def preRestart(reason: Option[Throwable], message: Option[Any]): IO[Unit] =
    for {
      _ <- IO.println(s"ParentActor is restarting due to: ${reason.fold("No Reason")(_.getMessage)}")
      _ <- super.preRestart(reason, message)
    } yield ()
  ...
  override def preSuspend(reason: Option[Throwable], message: Option[Any]): IO[Unit] =
    for {
      _ <- IO.println(s"ParentActor is being suspended due to: ${reason.map(_.getMessage).getOrElse("unknown reason")}")
      _ <- reason.fold(IO.unit)(t => IO(t.printStackTrace()))
      _ <- super.preSuspend(reason, message)
    } yield ()
...
case class ChildActor() extends Actor[IO] {
  ...
  override def preRestart(reason: Option[Throwable], message: Option[Any]): IO[Unit] =
    for {
      _ <- IO.println(s"ChildActor is restarting due to: ${reason.map(_.getMessage).getOrElse("unknown reason")}")
      _ <- super.preRestart(reason, message)
    } yield ()

  override def postRestart(reason: Option[Throwable]): IO[Unit] =
    for {
      _ <- IO.println(s"ChildActor has restarted due to: ${reason.map(_.getMessage).getOrElse("unknown reason")}")
      _ <- super.postRestart(reason)
    } yield ()
...
- In `ChildActor`, it prints a message indicating the reason for the restart and calls the superclass's `preRestart` method
  to ensure any default behavior is executed.
...
- In `ChildActor`, it prints a message indicating the reason for the restart and calls the superclass's `postRestart` method
  to ensure any default behavior is executed.
...
- In `ChildActor`, it prints a message indicating the reason for suspension, prints the stack trace if a Throwable is
  provided, and calls the superclass's `preSuspend` method to ensure any default behavior is executed.
...
- Raises an IllegalArgumentException with a message indicating that an illegal argument was provided and the actor will
  be stopped.
...
The retracted output from this run is as follows
...
When the "fail" message is receive the parent should restart the child actor
...
- **Termination Notifications**: If the watched actor stops, the watcher actor receives a Terminated message.
- **Unwatching**: An actor can stop watching another actor using the unwatch method.

### Example: Watching Actors

Let's define two actors: `ParentActor` and `ChildActor`. The `ParentActor` will watch the `ChildActor` and handle its
termination.
...
import com.suprnation.actor.Actor.Receive
import com.suprnation.actor.props.Props
import com.suprnation.actor.{Actor, ActorSystem, Terminated}
...
case class ParentActor() extends Actor[IO] {
  override def preStart: IO[Unit] =
    for {
      _ <- IO.println("ParentActor is starting.")
      child <- context.actorOf(Props(ChildActor()), "child-actor")
      _ <- context.watch(child) // Watch the child actor
    } yield ()
...
Next, let's create an `IOApp` to run the example. The `ParentActor` will watch the `ChildActor`, and we will send a message to
stop the `ChildActor` to see how the `ParentActor` handles the termination notification.
...
- In the `receive` method, it handles two types of messages:
  ...
  - `Terminated(ref)`: Handles the termination notification for the watched actor and prints a message.
...
- Sends a message to the `ParentActor` to stop the `ChildActor`.
- Waits for a short period to allow the messages to be processed.

## Scheduling messages

Let's create an example that demonstrates how to use the Scheduler class and its `scheduleOnce` method to schedule a task
to run after a specified delay.

### Example: Using Scheduler.scheduleOnce

In this example, we'll create an actor that schedules a task to print a message to the console after a delay of 5
seconds.
...
case class SchedulerActor() extends Actor[IO] {
  override def preStart: IO[Unit] =
...
Next, let's create an `IOApp` to run the example. We'll create an `ActorSystem` and a `SchedulerActor`, and then send a
message to observe the scheduling behavior.
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        ...
        schedulerActor <- system.actorOf(Props[IO](SchedulerActor()), "scheduler-actor")
...
- In the `preStart` method, `context.system.scheduler.scheduleOnce` is used to schedule a task to run after a delay of 5
  seconds. The task prints a message to the console.
- The `receive` method handles messages sent to the actor. In this example, it prints a message when it receives a
...
In an actor system, `become` and `unbecome` are used to change an actor's behavior dynamically at runtime. This allows
actors to handle different types of messages or states over time.

**Become**

- The `become` method changes the current behavior of the actor to a new behavior. This is useful for managing state
  transitions within an actor. Behaviour is changed on the next message.

**Unbecome**

- The `unbecome` method reverts the actor's behavior to the previous one. This is typically used to handle state
  transitions that need to revert back to an original or default behavior. Behaviour is changed on the next message.

### Example: Context `become` and `unbecome`

In this example, we'll create an actor that can switch between two behaviors: happy and angry. The actor will start in
the happy state and can switch to the angry state when it receives a specific message. It can also switch back to the
happy state.
...
case class MoodActor() extends Actor[IO] {
  def happy: Receive[IO] = {
    case "makeAngry" =>
  ...
  override def receive: Receive[IO] = happy
}
```

Next, let's create an `IOApp` to run the example. We'll create an `ActorSystem` and a `MoodActor`, and then send messages to
switch between states and observe the behavior.
...
object MoodActorExampleApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        moodActor <- system.actorOf(Props[IO](MoodActor()), "mood-actor")
...
This example demonstrates how to use `context.become` and `context.unbecome` to change an actor's behavior dynamically.
The `MoodActor` switches between happy and angry states based on the received messages, showcasing the flexibility of the
actor model in handling different behaviors.

## State Management - Ref's and PropsF
...
To instantiate an actor and create a `Ref` within its closure, we use `PropsF`. `PropsF` is a utility for creating actor properties (`Props`) where the factory method returns an effectful type. This is useful when actor creation involves side effects or asynchronous operations, allowing you to define actor creation within an effect context, such as `IO`.

### Example: Using Refs and PropsF

We'll create an actor that can switch between two behaviors: happy and angry. The actor will start in the happy state
and can switch to the angry state when it receives a specific message. It can also switch back to the happy state. We'll
use `Refs` to manage the state and PropsF to create the actor.
...
case class MoodActor(state: Ref[IO, String]) extends Actor[IO] {
  def happy: Receive[IO] = {
    case "makeAngry" =>
      for {
        _ <- IO.println("Switching to angry state...")
        _ <- state.set("angry")
        _ <- context.become(angry)
      } yield ()
    case "greet" =>
      IO.println("Hello! I'm happy!")
  }
...
object MoodActor {
  def props: IO[Props[IO]] =
    for {
      state <- Ref.of[IO, String]("happy")
    } yield PropsF(IO(MoodActor(state)))
}
```
...
object MoodActorExampleApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        moodActorProps <- MoodActor.props
        moodActor <- system.actorOf(moodActorProps, "mood-actor")
        _ <- moodActor ! "greet" // Should print: "Hello! I'm happy!"
        _ <- moodActor ! "makeAngry" // Should switch to angry state
        _ <- moodActor ! "greet" // Should print: "Go away! I'm angry!"
        _ <- moodActor ! "makeHappy" // Should switch back to happy state
...
- Defines two behaviors: happy and angry.
- The happy behavior handles two messages:
  - "makeAngry": Switches to the angry state using context.become and updates the `Ref`.
  - "greet": Prints a happy greeting.
- The angry behavior handles two messages:
  - "makeHappy": Switches back to the happy state using context.unbecome and updates the `Ref`.
  - "greet": Prints an angry greeting.
- The initial behavior is set to happy in the `receive` method.
...
- Returns a PropsF instance that creates a MoodActor with the Ref.
...
The helper method `MoodActor.props` is not necessary because the `actorOf` method can directly accept an instance of `Props[F]`. This allows the `MoodActor` to be defined inline, streamlining the actor creation process. Below is an example demonstrating this approach
...
The `context.setTimeout` method allows you to schedule a message to be sent to an actor after a certain period.

### Example: Using context.setReceiveTimeout

In this example, we'll create an actor that sets a receive timeout. If the actor does not receive any messages within
the specified duration, it will perform a timeout action.
...
case class TimeoutActor() extends Actor[IO] {
  override def preStart: IO[Unit] =
...
Next, let's create an `IOApp` to run the example. We'll create an `ActorSystem` and a `TimeoutActor`, and then send messages
to observe the timeout behavior.
...
object TimeoutActorExampleApp extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    actorSystemResource.use { system =>
      for {
        timeoutActor <- system.actorOf(Props[IO](TimeoutActor()), "timeout-actor")
        _ <- IO.sleep(3.seconds)
        _ <- timeoutActor ! "Hello" // Should print: "Received message: Hello"
        _ <- IO.sleep(6.seconds) // Should trigger the timeout action
        _ <- timeoutActor ! "Another message" // Should print: "Received message: Another message"
...
2. Creates an `ActorSystem` and a `TimeoutActor`.

- Sends a message to the `TimeoutActor` after 3 seconds, which should print the received message and reset the timeout.
...
Cats-Actors comes with a powerful Finite State Machine (FSM) out of the box, allowing you to manage complex actor
behaviors and state transitions seamlessly. By leveraging the FSM, you can define multiple states and transitions based
on received messages, ensuring your actors handle various scenarios effectively.

**Key Benefits**

- **State Management:** Easily define and manage multiple states within your actors.
- **Transitions:** Define transitions based on specific events, making your actor's behavior more predictable and
  maintainable.
- **Functional Programming:** Utilize the power of Cats Effect and functional programming principles for robust and clean
  state management.
...
First, define the states and data that the FSM will manage.
...
object ExampleFSMApp extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    val fsmBuilder = FSM[IO, State, Data]
      .withConfig(FSMConfig.withConsoleInformation[IO, State, Data])
      .when(Idle) {
        case (FSM.Event("start", data), stateManager) =>
          for {
            newState <- stateManager.goto(Active)
          } yield newState.using(data.copy(counter = data.counter + 1))
      }
      .when(Active) {
        case (FSM.Event("stop", data), stateManager) =>
          for {
    ...
    val fsm = fsmBuilder.startWith(Idle, Data(0))

    ActorSystem[IO]("example-fsm-system").use { system =>
      for {
        fsmActor <- system.actorOf(fsm.initialize)
        _ <- fsmActor ! "start"
        _ <- fsmActor ! "stop"
        _ <- fsmActor ! "start"
        _ <- fsmActor ! "stop"
...
The `amqpProducer` method is responsible for creating a producer that can send messages to a RabbitMQ queue. Here's a
...
4. **Message Publishing:** It returns a function that takes a message as input and publishes it to the queue using the
   channel.

#### amqpConsumerStream Method

The `amqpConsumerStream` method is responsible for creating a stream that consumes messages from a RabbitMQ queue. Here's
...
- Stream Construction:
  ...
  - Stream Composition: It combines the consuming stream with the queue stream to produce a continuous stream of
    messages.
...
```scala
actor
<- actorSystem.actorOf(Props(new Actor[IO] {
  override def receive: Receive[IO] = {
    case msg =>
      IO.println(msg)
  }
...
```scala
_
<- consumer.evalTapChunk(actor ! _).compile.drain.start
```

- `consumer`: This is the FS2 stream that consumes messages from RabbitMQ.
- `evalTapChunk(actor ! _)`: This method evaluates an effectful function for each chunk of the stream. Here, it sends each
  message to the actor using the ! operator.
- `compile.drain.start`: This compiles the stream to a IO and starts it concurrently.
...
```scala
actor
<- actorSystem.actorOf(PropsF(for {
  children <- Ref[IO].of[List[ActorRef[IO]]](List.empty)
  index <- Ref[IO].of[Int](1)
  maxChildren = 10
} yield new Actor[IO] {
...
`PropsF`: Creates an actor with a factory method that initializes state (children and index).
...
- Messages:
  ...
  - `ReceiveTimeout`: Sent if the actor is idle for a specified time, leading to an idle notification to the supervisor.
