package com.suprnation.samples.logic

import cats.effect.IO
import cats.implicits._
import com.suprnation.actor.Actor.Receive
import com.suprnation.actor._
import com.suprnation.actor.props.Props
import com.suprnation.typelevel.actors.implicits._

object Logic {
  case class AddComponent(wireName: String, actor: ActorRef[IO])

  case class StateChange(name: String, state: Boolean)

  /** This is not the best use of actors but it shows how one can migrate from Akka to Actors
    *
    * @param currentState
    *   the current state.
    */
  case class Wire(var currentState: Boolean) extends Actor[IO] with ActorLogging[IO] {

    var associations: Map[ActorRef[IO], String] = Map[ActorRef[IO], String]()

    override def receive: Receive[IO] = {
      case AddComponent(name: String, b: ActorRef[IO]) =>
        log(s"[Wire] Processing Message Adding Component $name ${b.path}") >>
          IO.delay {
            associations = associations + (b -> name)
          } >> log(s"[Wire] Added Association on $name for ${b.path}") >>
          (b ! StateChange(name, currentState))

      case GetValue => currentState.pure[IO]

      case s: Boolean =>
        if (currentState != s) {
          IO.delay {
            currentState = s
          } >>
            associations
              .map { case (ref: ActorRef[IO], name: String) =>
                log(
                  s"[Wire] Setting [State: $currentState] to [Name: $name] [Address: $ref}]"
                ) >> (ref ! StateChange(name, currentState))
              }
              .toList
              .sequence
              .as(s)
        } else IO(s)
    }

  }

  case class Inverter(input: ActorRef[IO], output: ActorRef[IO]) extends Actor[IO] with SimConfig {
    override def preStart: IO[Unit] = input ! AddComponent("in", self)

    override def receive: Actor.Receive[IO] = { case StateChange(_, s: Boolean) =>
      context.system.scheduler.scheduleOnce(inverterDelay) {
        output ! (!s)
      }
    }
  }

  case class Probe(input: ActorRef[IO]) extends Actor[IO] with ActorLogging[IO] {
    override def receive: Actor.Receive[IO] = {
      case add @ AddComponent(name: String, b: ActorRef[IO]) =>
        log(
          s"[Probe] Processing Message Adding Component ${name} ${b.path}"
        ) >> (input ?! add) >> log(s"[Probe] Sent message to ${b.path} ")
      case sc @ StateChange(_, currentState: Boolean) =>
        log(
          s"[Probe ${self.path.name}~>${input.path.name}] Wire is now $currentState"
        ) >> (input ?! sc)
      case b: Boolean =>
        // Here we will need to forward the message..
        log(
          s"[Probe ${self.path.name}~>${input.path.name}] Wire received [State: $b]"
        ) >> (input ?! b)
      case gv @ GetValue =>
        log(
          s"[Probe ${self.path.name}~>${input.path.name}] Current state is"
        ) >> (input ? [Boolean] gv)
    }
  }

  case class And(input0: ActorRef[IO], input1: ActorRef[IO], output: ActorRef[IO])
      extends Actor[IO]
      with ActorLogging[IO]
      with SimConfig {

    var in0: Option[Boolean] = None
    var in1: Option[Boolean] = None

    override def preStart: IO[Unit] =
      (input0 ! AddComponent("in0", self)) >>
        (input1 ! AddComponent("in1", self))

    override def receive: Actor.Receive[IO] = {
      case StateChange("in0", b: Boolean) =>
        IO {
          in0 = b.some
        } >> log(s"Setting IN 0 $b") >> sendMessage

      case StateChange("in1", b: Boolean) =>
        IO {
          in1 = b.some
        } >> log(s"Setting IN 1 $b") >> sendMessage
    }

    private def sendMessage: IO[Option[Boolean]] =
      (in0, in1)
        .mapN((first, second) =>
          context.system.scheduler.scheduleOnce(andDelay) {
            val result: Boolean = first && second
            (output ! result) >> IO(result)
          }
        )
        .sequence
        .flatTap(r => log(s"Sending Result $r"))
  }

  case class Or(input0: ActorRef[IO], input1: ActorRef[IO], output: ActorRef[IO])
      extends Actor[IO]
      with ActorLogging[IO]
      with SimConfig {

    var in0: Option[Boolean] = None
    var in1: Option[Boolean] = None

    override def preStart: IO[Unit] =
      (input0 ! AddComponent("in0", self)) >>
        (input1 ! AddComponent("in1", self))

    override def receive: Actor.Receive[IO] = {
      case StateChange("in0", b: Boolean) =>
        IO {
          in0 = b.some
        } >> sendMessage

      case StateChange("in1", b: Boolean) =>
        IO {
          in1 = b.some
        } >> sendMessage
    }

    private def sendMessage: IO[Option[Boolean]] =
      (in0, in1)
        .mapN((first, second) =>
          context.system.scheduler.scheduleOnce(andDelay) {
            val result: Boolean = first || second
            (output ! result) >> log(s"Sending $result") >> IO(result)
          }
        )
        .sequence
  }

  case class ForwardActor(input: ActorRef[IO]) extends Actor[IO] with ActorLogging[IO] {
    override def receive: Receive[IO] = { case msg: Any =>
      log("[Forwarder] Sending back!") >> (input >>! msg)
    }
  }

  case class EchoActor() extends Actor[IO] with ActorLogging[IO] {
    override def receive: Receive[IO] = { case msg: Any =>
      log("Sending back!") >>
        sender.fold(IO.unit)((x: ActorRef[IO]) => x ! msg).as(msg)
    }
  }

  case class OrAlt(input0: ActorRef[IO], input1: ActorRef[IO], output: ActorRef[IO])
      extends Actor[IO]
      with SimConfig {

    override def preStart: IO[Unit] = for {
      notInput0 <- context.actorOf(Props[IO](Wire(false)), "notInput0")
      notInput1 <- context.actorOf(Props[IO](Wire(false)), "notInput1")
      notOutput0 <- context.actorOf(Props[IO](Wire(false)), "notOutput0")

      _ <- context.actorOf(Props[IO](Inverter(input0, notInput0)), "A")
      _ <- context.actorOf(Props[IO](Inverter(input1, notInput1)), "B")
      _ <- context.actorOf(Props[IO](And(notInput0, notInput1, notOutput0)), "notAB")
      _ <- context.actorOf(Props[IO](Inverter(notOutput0, output)), "notnotAB")
    } yield ()
  }

  case class HalfAdder(a: ActorRef[IO], b: ActorRef[IO], s: ActorRef[IO], c: ActorRef[IO])
      extends Actor[IO]
      with SimConfig
      with ActorLogging[IO] {

    override def preStart: IO[Unit] = for {
      d <- context.actorOfWithDebug(Props[IO](Wire(false)), "d")
      e <- context.actorOfWithDebug(Props[IO](Wire(false)), "e")
      _ <- context.actorOfWithDebug(Props[IO](Or(a, b, d)), "Or")
      _ <- context.actorOfWithDebug(Props[IO](And(a, b, c)), "And")
      _ <- context.actorOfWithDebug(Props[IO](Inverter(c, e)), "Inverter")
      _ <- context.actorOfWithDebug(Props[IO](And(d, e, s)), "And2")
    } yield ()
  }

  case class FullAdder(
      a: ActorRef[IO],
      b: ActorRef[IO],
      cin: ActorRef[IO],
      sum: ActorRef[IO],
      cout: ActorRef[IO]
  ) extends Actor[IO]
      with ActorLogging[IO] {

    override def preStart: IO[Unit] = for {
      s <- context.actorOf(Props[IO](Wire(false)))
      c1 <- context.actorOf(Props[IO](Wire(false)))
      c2 <- context.actorOf(Props[IO](Wire(false)))
      _ <- context.actorOf(Props[IO](HalfAdder(a, cin, s, c1)), "HalfAdder1")
      _ <- context.actorOf(Props[IO](HalfAdder(b, s, sum, c2)), "HalfAdder2")
      _ <- context.actorOf(Props[IO](Or(c1, c2, cout)), "OrGate")
    } yield ()
  }

  case class Demux2(in: ActorRef[IO], c: ActorRef[IO], out1: ActorRef[IO], out0: ActorRef[IO])
      extends Actor[IO]
      with ActorLogging[IO] {
    override def preStart: IO[Unit] = for {
      notC <- context.actorOf(Props[IO](Wire(false)), "not")
      _ <- context.actorOf(Props[IO](Inverter(c, notC)), "InverterA")
      _ <- context.actorOf(Props[IO](And(in, notC, out1)), "AndGateA")
      _ <- context.actorOf(Props[IO](And(in, c, out0)), "AndGateB")
    } yield ()

  }

  case class Demux(in: ActorRef[IO], c: List[ActorRef[IO]], out: List[ActorRef[IO]])
      extends Actor[IO]
      with ActorLogging[IO] {

    /** This is pretty insane, we are going to compose the actor system using recursion... pwetttyyy kewwwlll
      *
      * @return
      */
    override def preStart: IO[Unit] = c match {
      case c_n :: Nil =>
        context.actorOf(Props[IO](Demux2(in, c_n, out.head, out(1)))).void

      case c_n :: c_rest =>
        for {
          out1 <- context.actorOf(Props[IO](Wire(false)))
          out0 <- context.actorOf(Props[IO](Wire(false)))
          _ <- context.actorOf(Props[IO](Demux2(in, c_n, out1, out0)))
          _ <- context.actorOf(Props[IO](Demux(out1, c_rest, out.take(out.size / 2))))
          _ <- context.actorOf(Props[IO](Demux(out0, c_rest, out.drop(out.size / 2))))
        } yield ()

      case Nil => IO.unit
    }

  }

  case object GetValue

}
