...
class TimerSpec extends AsyncFlatSpec with Matchers with TestKit {
  ...
  private def fixture(test: FixtureParams => IO[Unit]): Int = {
    ...
    Ref.empty[IO, Int].flatMap({ countRef =>
...
object TimersTest {
  ...
  case class TimedActor(
    ...
    override def receive: Receive[IO, TimerMsg] = {
      case StartTimer(key, delay, TimerMode.Single) => timers.startSingleTimer(key, CounterAdd(key), delay)
      case StartTimer(key, delay, TimerMode.FixedDelay) => timers.startTimerWithFixedDelay(key, CounterAdd(key), delay)
  ...
  case class ParentActor(
    ...
    override def receive: ReplyingReceive[IO, TimerMsg, Any] = {
      case msg => getOrCreateChild.flatMap(_ ! msg)
    }

    def getOrCreateChild: IO[ActorRef[IO, TimerMsg]] =
      for {
        childRefOpt <- timedActorRef.get
        ref <- childRefOpt.fold(context.actorOf(TimedActor(countRef, trackerActor, timerGenRef, timersRef)))(IO.pure)
